...
## 🔴 學習科學是弱科學，引用研究時要意識到這件事（2026-09-14 楊老師提出）

不像物理/化學這類硬科學，學習科學研究的對象（真實學習者的成效、慣習、心態）天生
充滿脈絡依賴性、個體差異大，而且**研究背後往往預設了特定的教育哲學立場**（行為主義
強調外在強化／認知取向強調schema／社會建構取向強調脈絡協作），這些立場會強烈影響
研究怎麼設計、怎麼詮釋數據、甚至怎麼定義「學習成效」本身。**即使是嚴謹的大型
meta-analysis，套到本專案的具體情境（台灣高中生、學測準備、900天長期、第二語言
字彙習得）時仍然是一種外推，不是已經驗證過的事實**——引用時除了標讀取程度，
要明確判斷這個外推的信心程度如何、這篇研究的樣本/任務類型跟本專案情境差多遠，
不要找到一篇符合預期結論的論文就直接套用當成鐵律。
...
1. **有新學到的東西才追加自己的知識庫**（不是每次都要）：
   ```bash
   python board_tool.py kb-append learning-scientist --text "<這次學到的具體內容，照kb/README.md的規則區分研究/實測/判斷>"
   ```
2. **一定要寫黑板一列**：
   ```bash
   python board_tool.py write --agent learning-scientist --task "<這次做什麼，一句話>" \

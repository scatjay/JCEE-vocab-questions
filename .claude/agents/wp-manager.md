...
- `ls .claude/agents/` 逐檔讀 frontmatter——11 隻現在都在不在、`tools`/`model` 有沒有寫全
...
- 新建／修改代理人之後，**用實際呼叫一次去驗證，不要用「應該可以了」帶過**——
  ...
  但 **2026-09-14 本線實測推翻了這條**：`probe-load-timing` 在建立它的同一個 session 裡
  直接呼叫成功，沒有重開。⇒ 兩種環境條件下的結果不一致，**不要假設哪一種現在成立**，
  每次新建/修改代理人後都用 `Agent(subagent_type: "<新代理人名>")` 或 `claude agents list`
  實際驗證一次再回報，不要照抄任何一條線的舊結論。
...
**跑完之後，在本線黑板 `.claude/agents/_runlog.jsonl` 追加一列**（只增不改；schema 見

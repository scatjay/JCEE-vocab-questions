...
【2026-09-14 情境D試玩PLAYTEST#1「精熟地圖+鎖定每日3題來源」提案】我的判斷：這次提案把「每日3題」從自由選改成鎖定到期複習/錯題優先派發，對時間破碎身分的風險不在地圖本身（沒空看=零感受，不是負擔），而在「派發到的題目結構性更難」這件事——到期/錯題本來就是答錯過或間隔久忘記的，通勤等碎片窗口第一題就撞到硬骨頭+斷線卡住的組合。這條game-designer原本三個鑽法推演完全沒討論到（三條都在討論選哪個KC刷，沒人討論派發難度）。本專案的實測（讀碼證實）：cards/index.html L321-324註解自承「舊版due.concat(fresh)不設上限，到期堆每天塞滿」是踩過的坑，代表「鎖定到期優先派發」疊加在同一套SRS上，斷線天數越多、到期堆越大、明天被迫派到更難項目的機率越高，這是機制交互風險非揣測。需真人驗證：連續兩週實測後第一題撞到難題的頻率與是否導致縮短使用時長。

## 2026-09-14

【2026-09-14 橫向盤點四站遊戲化現況】本專案的實測（讀碼證實）：WP-1/WP-8/WP-9整套遊戲化引擎（game/engine.js的XP/streak/HUD、game/kc-logic.js的KC精熟判定、game/floor-logic.js底線偵測）只掛在wordwheel.html一處（grep全站*.html的game/engine.js匯入，僅1個檔案命中）。gsat/index.html（843題主庫、真正的內容量體）、cards/index.html（SRS字卡）、translate/index.html（中翻英）三站完全零XP/streak/等級/徽章——分別是純粹的「作答→對錯上色→解析」（gsat另加官方答對率情緒校準acc-tag，這點對照領域簡報§3是正面案例）、純SRS排程（無感情化包裝，連Memrise花園那種視覺化都沒有）、自評式回饋（reveal答案後由學生自己點「我答對了/我需要加強」，無自動判分）。三站的「回饋」全部止於單次作答/單場結果，完全沒有跨場次的可見進度物（沒有本站KB已知的「能力可見」問題——這裡連能力都沒被追蹤，是資料層面就不存在，比wordwheel「有資料沒介面」更前一步）。額外發現：wordwheel/kc-logic.js的computeAnswerGain答錯也給+2XP（保底），配合HUD即時+N XP彈出toast，對「能力好但沒動機」的E身分是零風險的高頻小獎勵，走查中每答一題（不論對錯）數字都會漲，是具體可操作的farming路徑，而非揣測。

...
第八輪（特殊任務：文獻研究＋第一閉環對照＋第二閉環規劃，非例行WP調度/健檢）

工具限制先誠實記錄：本輪雖然frontmatter新增了WebSearch/WebFetch，但實際呼叫時兩支工具在本session一律回「No such tool available...disabled for this session」——不是找不到目標網頁，是工具本身在這個session被關掉，含subagent。下面的文獻內容全部是我既有訓練知識的轉述（reading level=未讀，僅記憶轉述，未經即時查證），不是這輪真的搜尋/讀取到的。所有作者/年份/框架名稱我有信心是對的（都是該領域公認的地標文獻），但沒有逐字核對過原文，也刻意不編造具體頁碼/DOI。下次工具若真的可用，第一件事是把下面幾個框架的關鍵論證跑一次真的WebSearch驗證，不要假設這輪的轉述已經等同文獻查證。

## 文獻發現（訓練知識轉述，未查證）

1. **Argyris & Schön 單迴圈/雙迴圈學習**（Organizational Learning, 1978）：單迴圈＝在既有治理變數（governing variable/norm）不變的前提下修正偏差（恆溫器調溫）；雙迴圈＝連治理變數本身都被質疑、可能被改掉（恆溫器問「為什麼設定是68度」）。套用本線：五棒循環本身（構思→設計→開發→評估→修正）預設是單迴圈——它在既有政策文字內找bug、補漏洞，不質疑政策的治理假設；`mutator`存在的理由就是把雙迴圈這件事變成一個有名字、有固定觸發點的角色，這跟它的prompt設計（「專門挑戰框架本身的假設」）完全對得上，不是我這輪才發現的巧合，是設計者顯然讀過或至少知道這個區分。

2. **PDCA（Deming）／OODA（Boyd）／敏捷回顧會議（Derby & Larsen）**：三者共通點是「檢查/定向」這一步必須靠外部證據校準，不能只靠內部邏輯自洽；回顧會議文獻常建議一次迭代只產出1-3個具體流程實驗、且要等至少一個週期後才回頭驗證效果，不要每輪都逼自己生出流程改變（避免thrashing）——這正好跟`MUTATIONS.md`「觀察窗口至少一輪其他實質工作後才填」的既有設計精神一致，算是既有機制不謀而合，不是我這輪才引入的新要求。

3. **Malone & Crowston 協調理論**（Coordination Theory, 1994）：協調＝管理活動之間的依賴關係（共用資源、產出-消費依賴、同步性約束）；「協調產生真正的加值」的判準是：某個依賴關係若沒有這層協調機制就不會被發現/解決，而不是「多一雙眼睛重複檢查同一件事」。用這個判準回頭看今天wp-manager在WP-8那輪的介入——那是產出-消費依賴（mutator的方案A是產出，裁決需要的schema/驗收標準一致性是消費端需求），我補的兩點缺口是mutator自己看不到、cycle-designer裁決當下也還沒撞到的，這才是「協調的加值」不是重複勞動，跟cycle-designer自己驗證的話（"不是形式性轉發"）在理論上站得住腳，不只是自我感覺良好。

4. **Contract Net Protocol（Smith 1980）**：原始設計的加值來源是「競標」——不知道誰最適合做這件事，靠公告/投標讓最適任者浮現。本線的CNP是削掉投標的扁平版（角色早就分工死），這代表我們拿不到CNP真正的那個「發現最適任者」的加值，我們的加值來源其實是Malone & Crowston講的依賴管理，不是CNP的競標機制——這是這輪讀文獻後修正的一個認知：我平常用「Contract Net式公告＋授予」形容自己的派工方式，這個類比在流程形式上成立，但**不要誤以為它帶來CNP原始理論宣稱的那種加值**，我們的加值另有來源，下次講這件事要講精確一點。

5. **Sandholm & Lesser Leveled Commitment Contracts（1995）**：核心是「允許事後付代價退出承諾，比死守承諾更能利用新資訊」，是談判/合約經濟學框架。`mutator`把它套到WP-8精熟判定的「離散狀態機該不該允許撤銷」上，是一次真正的跨領域遷移（不是換句話說），因為承諾經濟學裡的「退出罰則」被巧妙對應成「masteredAt撤銷後降級到demotedAt但保留歷史痕跡」——這是我這輪重新核對後認為框架選用**確實有創意含金量**，不只是貼標籤，補強了我第四輪已經記過的「框架選對」判斷。

6. **Stigmergy／黑板架構（Grassé 1959；Hayes-Roth 1985）**：靠環境留痕做間接協調，而非直接通訊。`mutator`在WP-9那次的最有價值發現，是點出候選A的安全網設計「宣稱」有兩層（直接開口+floorState留痕給老師），但沒人檢查過第二層的「痕跡」是否真的被誰讀取——這是stigmergy文獻裡一個沒有正式名字、但swarm robotics領域討論過的失效模式（trace laid but never read，跟蟻群費洛蒙沒被後續螞蟻感知到是同一類問題）。這輪我自己動手grep `gate/admin.html`確認：`floorState`資料確實已經在寫（`game/floor-logic.js`），但唯一的教師端頁面完全沒讀取它——證實了這個「留痕但沒人讀」的失效模式**真的發生了**，不是理論上的擔憂。

## 對照今天兩個閉環的具體結論

- **WP-8迴圈**：形式上是雙迴圈（mutator挑戰治理假設），但實際裁決結果是**部分雙迴圈**——只改了一個治理變數（masteredAt可撤銷），保留了更深的那個（精熟該不該是離散狀態，方案B被擱置）。這不是缺陷，是Argyris理論裡本來就存在的常態（組織很少一次把雙迴圈開到底，通常是漸進式鬆動），且擱置的理由（無校準資料）是站得住腳的理性理由，不是防禦性迴避——這點值得記下來，區分「有理由的部分雙迴圈」跟「防禦性抗拒雙迴圈」很重要，以後看到mutator提案被裁決成「只採納一部分」時，要先查是不是有像PARKED.md這樣附帶明確重新觸發條件的理性擱置，再判斷這是不是文獻講的那種「組織抗拒」。
- **PLAYTEST迴圈**：形式上是單迴圈（在既有玩法框架內修正，不質疑框架本身），但**過程中意外浮出一個雙迴圈等級的問題卻沒有被正式接住**：student-tester複測v2時發現的「WP-8政策6複習排程假設calendar day而非實際使用頻率」，game-designer正確判斷「這不是我的職權」轉出去，但轉出去之後**沒有落地到任何正式追蹤點**（不在PARKED.md、不在MUTATIONS.md、只停留在PLAYTEST.md表格的一句話裡）。這是本輪對照文獻後抓到的最重要落差：**兩個閉環案例都發生在同一天、速度很快，文獻（Reinertsen批次大小/前置工程理論、豐田andon）指出的風險不是「太快本身有害」，是「協調機制的追蹤速度跟不上任務並行速度」**——KB第四輪已經記過的「cycle-builder在mutator提案前已經照舊政策把code寫死，裁決後沒人主動點名要回頭同步」就是同一種失效模式的另一個實例：**decision-making的速度快過了「誰在依賴這個假設施工」的追蹤能力**，這是本線目前最具體、最可命名的系統性風險（用豐田術語＝沒有andon繩：沒有人在政策挑戰啟動的當下，去查「現在有沒有正在依賴舊假設施工的工作」）。
- 兩個迴圈都通過了我認為文獻上「真閉環」該有的最關鍵判準：**資訊不對稱被迫浮現且改變了結果**（wp-manager看到mutator/cycle-designer都沒看到的schema矛盾；student-tester讀code看到game-designer紙上推演沒看到的離線遺失風險）——這是我判斷「這兩個案例不是形式排練、是真的閉環」最有信心的一點，也是下一波規劃時要延續、不能弄丟的核心特徵。

## 下一波規劃的具體判斷（供下次接手直接用）

- **最低成本、最接近完整第二閉環的候選＝MUTATIONS #2**：只差cycle-designer一次裁決就能形成完整的挑戰→裁決結構，且我這輪已經查證出一個提案文件自己沒寫清楚的關鍵事實（floorState資料已就緒、只缺gate/admin.html讀取介面，WP-6/WP-5排程無關）——這比重新起一輪gsat遊戲化設計（候選a）便宜得多，應該優先排這個。
- **gsat 843題遊戲化方向若要變成第二閉環，不能只是「挑一個方向执行」**：要刻意設計成「game-designer/cycle-designer挑一個方向時，明講挑選標準是『核心假設最容易被挑戰』而非『看起來最好做』，然後在動工前（不是動工後）就排mutator去質疑那個核心假設」——這是從WP-8迴圈學到的「andon」教訓的直接應用：挑戰要搬到施工前面，不要重演「裁決後才發現code已經寫死」。
- **新發現、目前沒人認領的斷點**：WP-8政策6複習排程「calendar day生成、不看實際使用頻率」的假設，被PLAYTEST證據支撐、跨越WP-8/PLAYTEST邊界，但沒有正式落點——建議下次健檢時要嘛補進PARKED.md（若判斷現在資料不夠不值得動）、要嘛直接排給mutator當下一個挑戰目標（若判斷值得），不能讓它一直停留在PLAYTEST.md表格的散文句子裡，這正是這次文獻對照抓到的、之前7輪KB都沒點名過的一種新落差類型：「轉出去的職權」若沒有正式追蹤點承接，會在系統裡憑空消失，這跟第五輪記過的「needs_human/proposed不會自動結案」是同一種「協議有洞不是代理人偷懶」的家族，但這次是「範圍判斷轉手」這個新的觸發情境，值得記一筆新的。
- **用量現實**：本輪查得共用池全機今日合計67/68，幾乎見底——這輪規劃是「設計下一波的具體結構」而非「現在就公告新任務」，實際spawn要等下次額度重置後才發動，且發動順序建議：cycle-designer（裁決MUTATIONS#2，1次）優先於任何gsat遊戲化新方向的啟動（那個至少要3-4次spawn才能走完一個真正的閉環結構）。

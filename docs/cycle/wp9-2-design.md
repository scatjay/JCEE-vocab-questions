...
## 2026-09-14 `cycle-fixer` 修正：`showFloorHit()` 降階按鈕原地鬼打牆 bug

**發現者**：`student-tester`（情境C模擬）。**問題**：`showFloorHit()`（`wordwheel.html` 觸底畫面）
主按鈕原文案「回到單字輪重新開始（段考範圍）」承諾降階，但 `onclick` 只呼叫 `setMode('quiz')`，
完全沒有改變 `cur`（目前測驗的字族索引）——`setMode('quiz')` 只重建當前題池、重置計分
（`buildPool(); qi=0; right=0; done=0; ask();`）。也就是說，這顆按鈕實際上是把學生原地丟回**剛剛
讓他觸底的同一個字族、同一批題目**重新洗牌再考一次。因為 `evaluateFloorState` 規則3要求連續
答對5題才脫離觸底，學生原地重考同一批太難題目時，只要沒有連續答對5題，每答一題就會再看到一次
截斷畫面，形成「按鈕承諾降階、實際上困在原地反覆被截斷」的迴圈。

**根因是資料層，不只是接線漏了一步**：`DATA`（`wordwheel.html` 內建的12個字族）裡每個字族的
`items[].t` 欄位（`L1`-`L3`／`U9`-`U12`／日期／`第X級`）混雜在**同一個字族內部**，字族與字族
之間**沒有任何難度排序資料**——`docs/PLAN.md` §5 L586-588 定義的「段考範圍」是相對於**另一個
未建置的、學測級的站**而言（wordwheel 整站 vs 學測級整站），不是 wordwheel 內部 12 個字族之間
的難度分級。目前系統裡只有 wordwheel 這一站在跑，觸底偵測也是在 wordwheel 內部觸發，所以「降階
路由到 wordwheel.html（段考範圍）」這句話套用在「已經在 wordwheel.html 裡觸底的學生」身上時，
語意本身就是空的——沒有一個「更簡單的 wordwheel 版本」可以路由過去。**這是設計層留下的缺口
（單站部署 vs 多站降階路由假設的落差），不是這次能靠改程式碼解決的**，記錄於此供下一輪
`cycle-designer` 判斷是否要：(a) 幫 `DATA` 補上跨字族難度分級欄位，或 (b) 承認 wordwheel 目前是
唯一站、修改 §5 的降階敘述、或 (c) 等真正的學測級站建好後，觸底邏輯改成只在該站觸發、降階路由到
wordwheel。

**本次修法（最小可行修正，不是真正的降階）**：
1. 按鈕 onclick 改成隨機切到**另一個字族**（`cur` 排除自己後隨機取一個），避免原地鬼打牆。
2. 同時把「只做段考範圍的字」(`onlyExam`) 篩選勾選開啟，讓文案裡「段考範圍」四個字至少對應到
   程式碼真的會做的事（過濾掉大考中心補充字，只留課本/核心字彙/雜誌單字範圍的題目）。
3. 按鈕文案改為誠實描述「換一組單字重新開始（只考段考範圍）」，不再宣稱「回到單字輪」（因為學生
   本來就在單字輪站內，沒有另一個單字輪可回）或暗示這是有意義的「降階」（因為換到的下一個字族
   不保證比原字族簡單）。

**重驗（`cycle-fixer` 用 node 直接模擬邏輯，因專案沒有 puppeteer/jsdom，改用抽取 DATA + 純函式邏輯
模擬，非瀏覽器 DOM 端到端測試）**：
- 抽取 `wordwheel.html` 內建 `DATA`，確認 12 個字族。
- 模擬 1000 次點擊（起始 `cur=3`），驗證每次選到的字族都不等於觸發時的 `cur`（100% 不同）。
- 模擬連續 20 次觸底（每次都從上一次選到的字族再觸底一次），驗證序列裡沒有任何連續兩次選到同一
  字族（`any consecutive repeats: false`）——確認即使學生連續多次觸底，也不會落回同一批題目原地
  鬼打牆。
- `node --check` 驗證整段 `<script type="module">` 語法合法。

**殘留限制（沒有解決，誠實列出）**：
- 換到的字族**不保證更簡單**——只保證不是同一批題目。若學生剛好隨機換到另一個同樣難的字族，
  觸底可能再次發生（不是原本的無限迴圈，但也不是真正的信心重建）。`docs/PLAN.md` §5 L601-605
  已經點名「降階後前1-2題應優先選幾乎穩贏的內容」這件事屬於 `game-designer`/`ux-designer` 職權、
  尚未寫進驗收標準——這次的修法沒有處理這條，維持原狀待下一輪設計。
- 「wordwheel.html 是段考範圍」這句 §5 定案文字，在目前單站部署下對「已經在 wordwheel 裡觸底的
  學生」語意空洞，見上方根因分析——需要 `cycle-designer` 判斷怎麼處理，這次修正沒有動 `docs/PLAN.md`
  §5 本身的文字（不是我能單方面決定的設計問題）。
- 沒有端到端瀏覽器測試（DOM 渲染、按鈕實際可點、`onlyExam` checkbox 實際勾選狀態反映在畫面上）——
  這次重驗只到「邏輯層模擬」，真人測或無頭瀏覽器測試仍是殘留項目。

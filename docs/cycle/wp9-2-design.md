...
> **重要提醒（給 `cycle-builder`／下一個讀者）**：`cycle-builder` 目前正在背景平行施工 WP-9 Phase 1-3，
> 依授權時的交代「文案原封不動放進 UI」使用的是**v1 原文**（本文件已標記為棄用，見下方）。
> **UI 裡的候選 A 文字需要在 cycle-builder 這輪完工後，再做一次文字替換成 v2**——這兩輪是平行跑的，
> cycle-builder 看不到這次的文字修訂，不能假設它已經是新版。
...
| | 文案 | 優點 | 缺點/風險 |
|---|---|---|---|
| A（保守／學業導向，**v2，2026-09-14 english-teacher 審查後修訂，詳見下方「2026-09-14 v2」小節**） | 「這一段的題目，目前對你來說太難了——這不是你的問題，是這裡的內容本來就是這個難度。下次上課，你可以直接跟英文老師說：『我在這個系統裡一直卡關，想請你幫我看看接下來怎麼繼續。』他會比系統更清楚怎麼幫你。」 | 不需要系統知道任何個資／學校配置；不把「單字太難」跟「心理狀態」掛勾，避免不成比例的框架；「老師」是每個學生都有的通用角色；**v2 額外做到**：把限制歸給內容本身而非「你的能力範圍」（避免委婉包裝的責備語氣）、給可直接照唸的求助台詞（降低「不知道怎麼講」的門檻）、與畫面上同時出現的降階路由按鈕（wordwheel/roots）不互相矛盾。 | 若學生剛好跟老師關係疏離、求助不到人，這句話可能落空、沒有真正的兜底（風險與 v1 相同，未改變）。 |
...
**本輪定案（可以現在進 build 的部分）**：v1 上線用 **候選 A**（現為 v2 文字，見上表）。理由：它已經滿足 red-team
...
**驗收標準 3(b) 現況更新**：v1 候選 A 已完成一輪 Phase 4 真身審查，結果**不通過**，已依上述修訂為 v2。
**v2 本身尚未經 `english-teacher`／`red-team-critic` 逐字覆核**（本輪是 `cycle-designer` 依他的分析
自行修正，多了一個他審查範圍外的畫面一致性修正）——驗收標準 3(b) 在 v2 文字上**仍未關閉**，
建議下一輪排一次針對 v2 這一小段文字的快速確認（不需要重新整輪審查），確認新文字本身沒有引入
新的語氣問題。
...
3.（負，本文件新增，取代原本只靠關鍵字黑名單的版本）觸發訊息（候選 A 或楊老師選定版本）
   ...
   **狀態更新（2026-09-14）**：v1 文字已跑過 (b)，結果不通過；已修訂為 v2；v2 尚未跑過 (b)，
   這條驗收標準在 v2 文字上仍未關閉。
...
3. （沿用既有、未變動）§7 待決事項 #9：字根表降階的先備條件門檻怎麼量化——本輪仍不處理，
   維持只加提醒文案的做法。
4. **候選 A 的逐字文案已完成一輪 Phase 4 真身審查**（`english-teacher`，結果不通過，已修訂為
   v2，見上方「2026-09-14 v2」小節）。**仍待辦**：v2 這一小段新文字本身尚未經任何人真身覆核，
   建議下一輪排一次針對 v2 的快速確認（不是重新整輪審查）——這不是「等答案才能問」的問題，
   是需要 `wp-manager` 之後排工排進去的一個步驟，本文件先點名，避免又被漏排。
...
**`cycle-builder` 目前平行施工用的是 v1 原文，完工後需要再做一次文字替換成 v2**——這是本輪
唯一新增的、必須有人接手完成的動作，不是設計層面還沒定案。
